...
The `figma-cli` is a Rust CLI binary (repo root). It wraps the same Figma REST API operations available via the figma-console-mcp MCP server as shell commands. Use it for scripting, CI/CD automation, batch processing, and performance comparison with MCP tools.
...
```bash
cargo build --release
# Binary at: target/release/figma-cli
